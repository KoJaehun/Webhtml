var joinValidate = {
	...
	resultCode : {
		...
		invalid_pw : {
			code: 3,
			desc : '비밀번호는 8자 이상이어야 하며, 숫자/대문자/소문저/특수문자를 모두 포함해야 합니다.'
		},
		stream_pw : {
			code: 4,
...
// 아이디 유효성 체크 
	...
	var regEtc = /[~`!@#$%^&*()+=\|\\\{\}\[\]:";'<>.,?//]/g; 		// 특수문자 사용X
	var regId = /[^a-z0-9-_.]+/g; 									// 올바른 아이디 형식


	if(id == '' || id.length == 0) {							// 1. 값이 있는지 체크
			return this.resultCode.empty_val;
			
		} else if(id.match(regEmpty)) {							// 2. 값사이에 공백
		...
		} else if(id.charAt(0) == '-' || id.charAt(0) == '_') { // 5. 첫글자 특수문자허용 X
